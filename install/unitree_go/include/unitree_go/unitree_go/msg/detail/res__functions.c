...
unitree_go__msg__Res__create(void)
{
  ...
  unitree_go__msg__Res__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    unitree_go__msg__Res * data =
      (unitree_go__msg__Res *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!unitree_go__msg__Res__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          unitree_go__msg__Res__fini(&output->data[i]);
        }
        return false;
      }
    }
